...
#	define CHECK_GL_ERROR
	...
	uint32 RenderDeviceGL4::createTexture(TextureTypes::List type, int width, int height, int depth, TextureFormats::List format, bool hasMips, bool genMips, bool compress, bool sRGB)
	{
		assert1(depth > 0);
	...
	uint32 RenderDeviceGL4::createShaderProgram(const char *vertexShaderSrc, const char *fragmentShaderSrc, const char *geometryShaderSrc, const char *tessControlShaderSrc, const char *tessEvalShaderSrc, const char *computeShaderSrc)
	{
		int infologLength = 0;
		int charsWritten = 0;
		char *infoLog = 0x0;
	...
	uint32 RenderDeviceGL4::createShader(const char *vertexShaderSrc, const char *fragmentShaderSrc, const char *geometryShaderSrc, const char *tessControlShaderSrc, const char *tessEvaluationShaderSrc, const char *computeShaderSrc)
	{
	...
	uint32 RenderDeviceGL4::createRenderBuffer(uint32 width, uint32 height, TextureFormats::List format, bool depth, uint32 numColBufs, uint32 samples)
	{
	...
	bool RenderDeviceGL4::getRenderBufferData(uint32 rbObj, int bufIndex, int *width, int *height, int *compCount, void *dataBuffer, int bufferSize)
	{
		int x, y, w, h;
		int format = GL_RGBA;
		int type = GL_FLOAT;
	...
	bool RenderDeviceGL4::applyVertexLayout(RDIGeometryInfoGL4 &geo)
	{
		...
		for (uint32 i = 0; i < vl.numAttribs; ++i)
		{
			...
			if (attribIndex >= 0)
			{
				...
				assert1(_buffers.getRef(geo.vertexBufInfo[attrib.vbSlot].vbObj).glObj != 0 &&
	...
	void RenderDeviceGL4::drawIndexed(RDIPrimType primType, uint32 firstIndex, uint32 numIndices, uint32 firstVert, uint32 numVerts)
	{
		if (commitStates())
		{
			firstIndex *= (_indexFormat == IDXFMT_16) ? sizeof(short) : sizeof(int);

			glDrawRangeElements(RDI_GL4::primitiveTypes[(uint32)primType], firstVert, firstVert + numVerts,	numIndices, RDI_GL4::indexFormats[_indexFormat], (char*)0 + firstIndex);
		}
