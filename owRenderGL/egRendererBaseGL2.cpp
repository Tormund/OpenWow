...
#	define CHECK_GL_ERROR
	...
	{
		OW_UNUSED_VAR(geometryShaderSrc);
		OW_UNUSED_VAR(tessControlShaderSrc);
		OW_UNUSED_VAR(tessEvaluationShaderSrc);
		OW_UNUSED_VAR(computeShaderSrc);
	...
	void RenderDeviceGL2::setRenderBuffer(uint32 rbObj)
	{
		...
		else
		{
			...
			assert1(glCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT) == GL_FRAMEBUFFER_COMPLETE_EXT);
			_fbWidth = rb.width;
			_fbHeight = rb.height;
