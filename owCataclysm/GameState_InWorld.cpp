...
void GameState_InWorld::InputPhase(double t, double dt)
{
    float speed = 4.5f;
...
MOUSE_MOVED_(GameState_InWorld)
{
    if (enableFreeCamera)
    {
...
MOUSE_PRESSED(GameState_InWorld)
{
...
MOUSE_RELEASE(GameState_InWorld)
{
    if (_button == OW_MOUSE_BUTTON_LEFT)
    {
        enableFreeCamera = false;
...
MOUSE_WHEEL(GameState_InWorld)
{
    return false;
}

KEYBD_PRESSED(GameState_InWorld)
{
...
KEYBD_RELEASE(GameState_InWorld)
{
    if (_key == OW_KEY_X)
    {
        cameraSprint = false;
