...
void MapChunk::init(vector<Texture*>* mt, File& f, load_phases phase)
{
	...
	if (phase == main_file)
	{
		...
		// Buffers
		{
			...
			glBufferSubData(GL_ARRAY_BUFFER, C_MapBufferSize * 3 * sizeof(float), C_MapBufferSize * 2 * sizeof(float), _MapsManager->dataDetail);
			glBufferSubData(GL_ARRAY_BUFFER, C_MapBufferSize * 5 * sizeof(float), C_MapBufferSize * 2 * sizeof(float), _MapsManager->dataAlpha);
		...
		f.Seek(header->ofsAlpha);
		{
			...
			if (header->nLayers > 0)
			{
				for (uint32_t i = 1; i < header->nLayers; i++)
				{
					...
					if (_MapsManager->GetMap()->IsBigAlpha() && ((mcly[i].flags & MCLY::FLAG_ALPHA_MAP_COMRESSED) == MCLY::FLAG_ALPHA_MAP_COMRESSED))
					...
					else if (_MapsManager->GetMap()->IsBigAlpha())
					{ // Uncomressed (4096)
...
void MapChunk::draw2(MapTilePass* pass)
{
	visible = false;
	...
	if (!hasholes)
	{
		...
		if (highres)
		{
			strip = _MapsManager->mapstrip2;
			striplen = stripsize2;
		}
		else
		{
			strip = _MapsManager->mapstrip;
			striplen = stripsize;
		}
	}
	...
	pass->SetMCCVExists(header->flags.has_mccv && _Settings->enableMCCV);
	pass->SetMCLVExists(MCLV_exists && _Settings->enableMCLV);
	...
	if (header->flags.has_mcsh)
	{
		pass->SetShadowColor(_EnvironmentManager->GetSkyColor(SHADOW_COLOR) * 0.3f);
	}
...
void MapChunk::drawNoDetail()
{
	...
	glDrawElements(GL_TRIANGLE_STRIP, stripsize, GL_UNSIGNED_SHORT, _MapsManager->mapstrip);
