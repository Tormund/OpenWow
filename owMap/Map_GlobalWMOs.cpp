...
{
	// Delete global WMO
	delete globalWMOplacementInfo;
	if (globalWMOExists)
	{
		_WMOsMgr->Delete(globalWMOName);
	}

	// Delete Low-resolution wmo
	ERASE_VECTOR(lowResolutionWMOsplacementInfo);
...
void Map_GlobalWMOs::AddLowResWMOName(cstring _name)
{
	...
	Debug::Green("Map_GlobalWMOs: Low-res name [%s]", _name.c_str());
}
