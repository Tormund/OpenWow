...
bool TexturesMgr::LoadSoilTexture(ByteBuffer& _buffer, Texture* _texture) {
...
bool TexturesMgr::LoadBLPTexture(ByteBuffer& _buffer, Texture* _texture) {
	_texture->Bind();
	...
	if(header.colorEncoding == 2) {
		...
		for(int i = 0; i < 16; i++) {
			...
			if(header.mipOffsets[i] && header.mipSizes[i]) {
				_buffer.Seek(header.mipOffsets[i]);
				_buffer.ReadBytes(buf, header.mipSizes[i]);
	...
	else if(header.colorEncoding == 1) {
		unsigned int pal[256];
		_buffer.ReadBytes(pal, 1024);
		...
		bool hasalpha = header.alphaSize != 0;
